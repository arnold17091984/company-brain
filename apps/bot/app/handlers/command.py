...
from app.formatters.response import escape_markdown, format_answer, format_error, format_sources
...
async def handle_search(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not query:
        await update.message.reply_text(
            escape_markdown("Please provide a search term. Example: /search onboarding checklist"),
            parse_mode=ParseMode.MARKDOWN_V2,
        )
        return
    ...
    text = escape_markdown(f"Top results for: {query}") + "\n\n" + format_sources(response.sources)
...
async def handle_lang(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Usage: /lang <en|ja|ko|tl>
    Alias: /language <en|ja|ko|tl>
    ...
    current_lang: str = (context.user_data or {}).get("language") or settings.default_language
    args = context.args or []
    ...
    lang_name = _SUPPORTED_LANGUAGES[code]
    confirmation = escape_markdown(t("language_set", lang=code, language=lang_name))
    await update.message.reply_text(confirmation, parse_mode=ParseMode.MARKDOWN_V2)

    # Suppress unused variable warning – current_lang used for context only.
    _ = current_lang


async def handle_history(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show a summary of the user's recent conversation history.

    Fetches the conversation summary from the API using the stored
    ``conversation_id``. If no conversation has started yet, informs the user.
    ...
    lang: str = (context.user_data or {}).get("language") or settings.default_language
    conversation_id: str | None = (context.user_data or {}).get("conversation_id")

    if not conversation_id:
        await update.message.reply_text(
            escape_markdown(t("history_empty", lang)),
            parse_mode=ParseMode.MARKDOWN_V2,
        )
        return
    ...
    try:
        ...
        logger.error("API error during /history: %s", exc, exc_info=True)
        await update.message.reply_text(
            format_error("History is temporarily unavailable. Please try again later."),
            parse_mode=ParseMode.MARKDOWN_V2,
        )
        return
    except Exception as exc:
        logger.exception("Unexpected error during /history: %s", exc)
        await update.message.reply_text(
            format_error("An unexpected error occurred."),
            parse_mode=ParseMode.MARKDOWN_V2,
        )
...
async def handle_clear(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Reset the user's conversation by clearing the stored conversation ID.
    ...
    await update.message.reply_text(
        escape_markdown(t("conversation_cleared", lang)),
        parse_mode=ParseMode.MARKDOWN_V2,
    )
