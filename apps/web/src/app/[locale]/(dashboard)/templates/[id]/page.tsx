...
export default function TemplateDetailPage({ params }: { params: { id: string; locale: string } }) {
	...
	const handleVote = async () => {
		try {
			const res = await fetch(`${API_BASE_URL}/api/v1/templates/${params.id}/vote`, {
				method: "POST",
				headers: { Authorization: `Bearer ${getToken()}` },
			});
			if (!res.ok) return;
			setVoted((v) => !v);
			setTemplate((prev) =>
				prev ? { ...prev, vote_count: voted ? prev.vote_count - 1 : prev.vote_count + 1 } : prev,
			);
		} catch {
			// silent
		}
	...
	const handleCopy = async () => {
		try {
			const res = await fetch(`${API_BASE_URL}/api/v1/templates/${params.id}/copy`, {
				method: "POST",
				headers: { Authorization: `Bearer ${getToken()}` },
			});
			if (!res.ok) return;
			setCopied(true);
			setTemplate((prev) => (prev ? { ...prev, copy_count: prev.copy_count + 1 } : prev));
		} catch {
			// silent
		}
	};
	...
	const categoryColor = template
		? (CATEGORY_COLORS[template.category] ?? "bg-stone-50 text-stone-600 border-stone-200 dark:bg-stone-700 dark:text-stone-300 dark:border-stone-600")
		: "";
	...
	if (error || !template) {
		return (
			<div className="flex flex-col h-full">
				<div className="border-b border-stone-200/60 dark:border-stone-700/60 bg-white/80 dark:bg-stone-900/80 backdrop-blur-sm px-6 py-4 shrink-0">
					<Link href="../templates" className="inline-flex items-center gap-1.5 text-sm text-stone-500 hover:text-stone-700 dark:text-stone-400 dark:hover:text-stone-200 transition-colors">
				...
				<div className="max-w-3xl mx-auto space-y-6">
					...
					<div className="bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 p-6 shadow-sm">
						<div className="flex items-start justify-between gap-4 mb-3">
							...
							<span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border shrink-0 ${categoryColor}`}>
						...
						<div className="flex items-center justify-between flex-wrap gap-3 pt-3 border-t border-stone-100 dark:border-stone-700">
							<div className="flex items-center gap-4 text-xs text-stone-500 dark:text-stone-400">
								<span>{t("by")} {template.author_name}</span>
							...
							<div className="flex items-center gap-2">
								...
								>
									<svg className="w-4 h-4" fill={voted ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
