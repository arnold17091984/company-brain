...
const COLUMNS: Column[] = [
	...
	{ key: "date", label: "Date", align: "left", format: (v) => new Date(v as string).toLocaleDateString() },
	{ key: "query_count", label: "Queries", align: "right" },
	{ key: "total_input_tokens", label: "Input Tokens", align: "right", format: (v) => Number(v).toLocaleString() },
	{ key: "total_output_tokens", label: "Output Tokens", align: "right", format: (v) => Number(v).toLocaleString() },
	{ key: "avg_latency_ms", label: "Avg Latency", align: "right", format: (v) => `${Math.round(Number(v))} ms` },
...
function SortIcon({ dir }: { dir: SortDir | null }) {
	if (!dir) {
		return (
			<svg className="w-3 h-3 text-stone-300 dark:text-stone-600 opacity-0 group-hover:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
	...
	if (dir === "asc") {
		return (
			<svg className="w-3 h-3 text-indigo-600 dark:text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
...
export function UsageMetricsTable({ data }: UsageMetricsTableProps) {
	...
	if (data.length === 0) {
		return (
			<div className="flex items-center justify-center py-12 text-sm text-stone-400 dark:text-stone-500" role="status">
									...
									return (
										<td key={col.key} className="px-4 py-3 text-right">
											<span className="inline-flex items-center gap-1 text-red-500 dark:text-red-400 font-medium">
												<svg className="w-3.5 h-3.5 rotate-180" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
													<path d="M7.493 18.75c-.425 0-.82-.236-.975-.632A7.48 7.48 0 016 15.375c0-1.75.599-3.358 1.602-4.634.151-.192.373-.309.6-.397.473-.183.89-.514 1.212-.924a9.042 9.042 0 012.861-2.4c.723-.384 1.35-.956 1.653-1.715a4.498 4.498 0 00.322-1.672V3a.75.75 0 01.75-.75 2.25 2.25 0 012.25 2.25c0 1.152-.26 2.243-.723 3.218-.266.558.107 1.282.725 1.282h3.126c1.026 0 1.945.694 2.054 1.715.045.422.068.85.068 1.285a11.95 11.95 0 01-2.649 7.521c-.388.482-.987.729-1.605.729H14.23c-.483 0-.964-.078-1.423-.23l-3.114-1.04a4.501 4.501 0 00-1.423-.23h-.777zM2.331 10.977a11.969 11.969 0 00-.831 4.398 12 12 0 00.52 3.507c.26.85 1.084 1.368 1.973 1.368H4.9c.445 0 .72-.498.523-.898a8.963 8.963 0 01-.924-3.977c0-1.708.476-3.305 1.302-4.666.245-.403-.028-.959-.5-.959H4.25c-.832 0-1.612.453-1.918 1.227z" />
												</svg>
												{displayed}
											</span>
